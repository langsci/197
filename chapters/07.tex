...
With o/el conceptual tools, there are two types of accounts of \isi{ellipsis}, one motoric and the other syntactic. This is also the case for conventional approaches. For several reasons the syntactic account is preferable, but lets consider both approaches for the sake of argument. 
...
\end{figure}
  ...
  The assisted \isi{persistence mechanism} is highly constrained. \{A\textsc{ux}\} can allow both \{V\} and \{-N\} to persist across a clausal reorganization \REF{ex:7:4a}, but the trajectory is somewhat less coherent if only \{V\} persists \REF{ex:7:4b}. Coherence is strongly compromised when if only \{-N\} or \{+N\} are required to persist above ground, as in \REF{ex:7:4c} and \REF{ex:7:4d}. This suggests that relative e-organization may be a factor in assisted persistence and that \isi{coherence} intuitions will differ in languages with different fixed word orders. 
...
Another influence on \isi{coherence} of trajectories with \isi{ellipsis} is excitation of systems by discourse context. Contextually excited cs-systems which, despite not being selected in the same sentence, and perhaps never being selected at all, can participate in ϕ-configurations if they have above-ground excitation. Consider the answer \isi{ellipsis} examples in \REF{ex:7:8}:
...
Direct reference and \isi{ellipsis} create \isi{relational meaning} experiences from ϕ-configurations directly through coupling of lexical s-systems. In contrast, \isi{anaphora} creates \isi{relational meaning} more indirectly. For example, by coupling to both [Al]\{N\} and [drinks]\{V\}, \{\textsc{pro}\} indirectly brings about the ϕ-configuration {\textbar}Al drinks coffee{\textbar}. In \isi{direct reference} and \isi{anaphora}, the relevant cs-system which participates in the ϕ-configuration(s) is selected; in \isi{ellipsis}, this cs-system is not selected. Given this taxonomy of reference, a logical possibility is a non-selected [\textsc{pro}]\{\textsc{pro}\} system giving rise to a ϕ-configuration indirectly. This could correspond to a null \isi{pronominal} or big \textsc{pro}, as in \textit{Al wants} \textsc{pro} \textit{to drink coffee}.
...
The analysis of island phenomena we develop here proposes two different mechanisms for islands. We examine only a subset of island types, and our aim is fairly modest: to provide a starting point for a more comprehensive theory. The first mechanism applies to cases in which a ϕ-configuration cannot cohere because the relevant [\isi{wh}] system is grounded by an intervening reorganization, rather than persisting in an \isi{excited state}. This applies to adjuncts, wh-clauses, and relative clauses, which are exemplified below. In contrast, \textit{that} complement clauses and bare complements do not strongly ground, and hence do not give rise to island effects. These differences are summarized in {\tabref{tab:7:5}}:

\begin{table}
\small
\begin{tabularx}{\textwidth}{llQ}
\lsptoprule
\multicolumn{3}{c}{\textbf{Strongly \isi{grounding reorganization}: island effects}}\\
\midrule
\isi{adjunct} island \\
...
\tablevspace
\multicolumn{3}{c}{\textbf{Non-\isi{grounding reorganization}: no island effects}}\\
\midrule 
...
\end{table}
  ...
  An example island trajectory is shown in {\figref{fig:7:19}}. The adjunctive reorganization Ê\textsubscript{4} is strongly grounding. Hence [what]\{N\} is grounded by Ê\textsubscript{4}, and the configuration in (e4) does not cohere because not all of the systems in the relevant ϕ-configuration ({\textbar}Al drinks what{\textbar}) are excited. With some effort, [what]\{N\} can be made to persist above ground through Ê\textsubscript{4}, despite this adjunctive reorganization being a strongly grounding one. “Effort” of this sort is almost always a relevant source of variation in \isi{coherence} intuitions for islands, and below we consider how persistence can account for “exceptions” to the island patterns described here.
